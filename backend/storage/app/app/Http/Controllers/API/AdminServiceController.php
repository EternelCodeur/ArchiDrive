...
class AdminServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Ensure a root folder entry exists for this service
        try {
            $existsRoot = Folder::where('service_id', $service->id)->whereNull('parent_id')->first();
            if (!$existsRoot) {
                Folder::create([
                    'name' => $service->name,
    ...
    public function destroy(Service $service)
    {
        ...
        // Delete service folders (and their children via FK cascade on parent_id)
        try {
            Folder::where('service_id', $service->id)->delete();
        } catch (\Throwable $e) {}
    ...
    public function visible(Request $request)
    {
        ...
        if ($user->role === 'agent') {
            ...
            if ($serviceId) {
                $service = Service::where('enterprise_id', $enterpriseId)->where('id', $serviceId)->get();
                return response()->json($service);
            }
            return response()->json([]);
        }
